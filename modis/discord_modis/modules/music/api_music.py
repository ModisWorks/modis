...
def build_yt_api():
    ...
    try:
        ...
        ytdiscoveryapi = googleapiclient.discovery.build("youtube", "v3",
...
def build_sc_api():
    ...
    if "soundcloud_client_id" not in data["discord"]["keys"]:
        ...
        logger.info(
            "Please add your SoundCloud client id with name "
            "'soundcloud_client_id' in data.json to use Soundcloud features "
            "of the music module")
        return False
...
def build_spotify_api():
    ...
    if "spotify_client_secret" not in data["discord"]["keys"]:
        ...
        logger.info(
            "Please add your Spotify client secret with name "
            "'spotify_client_secret' in data.json to use Spotify features of "
            "the music module")
        return False
...
def parse_query(query, ilogger):
    ...
    if args[0].lower() in ["sp", "spotify"] and spclient is not None:
        ...
        try:
            if len(args) > 2 and args[1] in ['album', 'artist', 'song', 'track',
                                             'playlist']:
            ...
            ilogger.info(
                "Queueing Spotify {}: {}".format(query_type, query_search))
            spotify_tracks = search_sp_tracks(query_type, query_search)
            if spotify_tracks is None or len(spotify_tracks) == 0:
                ilogger.error("Could not queue Spotify {}: {}".format(
                    query_type, query_search))
                return []
            ilogger.info(
                "Queued Spotify {}: {}".format(query_type, query_search))
        ...
        try:
            requests = ['song', 'songs', 'track', 'tracks',
                        'user', 'playlist', 'tagged', 'genre']
            ...
            query_type = query_type.replace('song', 'track')
            ilogger.info(
                "Queueing SoundCloud {}: {}".format(query_type, query_search))
            soundcloud_tracks = search_sc_tracks(query_type, query_search)
            ilogger.info(
                "Queued SoundCloud {}: {}".format(query_type, query_search))
...
def get_ytvideos(query, ilogger):
    ...
    if search_result["items"][0]["id"]["kind"] == "youtube#video":
        ...
        queue.append(
            ["https://www.youtube.com/watch?v={}".format(videoid), title])

    # Queue playlist if playlist
    elif search_result["items"][0]["id"]["kind"] == "youtube#playlist":
        queue = get_queue_from_playlist(
            search_result["items"][0]["id"]["playlistId"])
...
def get_queue_from_playlist(playlistid):
    ...
    for entry in playlist["items"]:
        ...
        queue.append(
            ["https://www.youtube.com/watch?v={}".format(videoid), songname])
...
def search_sc_tracks(query_type, query_search):
    results = []
    if query_type == 'track':
        results = scclient.get("/tracks", q=query_search, filter="public",
                               limit=1)
        ...
        results = scclient.get("/tracks", tags=query_search, filter="public",
                               limit=50)
    elif query_type == 'genre':
        while ", " in query_search:
            query_search = query_search.replace(", ", ",").strip()
        results = scclient.get("/tracks", genres=query_search, filter="public",
                               limit=50)
...
def search_sp_tracks(query_type, query_search):
    ...
    if query_type == 'track':
        ...
        if "playlists" in results:
            if "items" in results["playlists"] and len(
